...
struct CardFormView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("Số thẻ").fontWeight(.bold)
            HStack {
                TextField("Số thẻ", text: $cardNumberText)
            ...
            Text("Ngày hết hạn").fontWeight(.bold)
            TextField("Ngày hết hạn", text: $expirationDateText)
                .onChange(of: expirationDateText) { newValue in
                    expirationDateText = cardFormatter.formatFieldWith(newValue, field: .expirationDate)
                }
            Text("Mã bí mật").fontWeight(.bold)
            TextField("Mã bí mật", text: $cvvText)
